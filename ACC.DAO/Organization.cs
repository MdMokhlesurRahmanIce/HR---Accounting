...
namespace ACC.DAO
{
    ...
    public class Organization : BaseItem
    {
        ...
        public static CustomList<Organization> GetAllOrgByParentKey(long orgkey)
        {
            ConnectionManager conManager = new ConnectionManager(ConnectionName.HR);
            CustomList<Organization> Gen_OrgCollection = new CustomList<Organization>();
            IDataReader reader = null;
            String sql = string.Format("Select CompanyName=Left(OrgCode,2),BranchName=Left(OrgCode,4),* from Gen_Org where orgparentkey = {0}", orgkey);
            try
            {
                conManager.OpenDataReader(sql, out reader);
                while (reader.Read())
                {
                    Organization newGen_Org = new Organization();
        ...
        public static CustomList<Organization> GetAllOrganization(string empCode, Int32 isAdmin)
        {
            ConnectionManager conManager = new ConnectionManager(ConnectionName.HR);
            CustomList<Organization> OrganizationCollection = new CustomList<Organization>();
            IDataReader reader = null;
            string search = "'" + empCode + "'," + isAdmin;
            String sql = "EXEC GetUserWiseCompanyList " + search + "";
            //String sql = "select CompanyName=Left(OrgCode,2),BranchName=Left(OrgCode,4),* from Gen_Org where LEN(OrgCode)=" + codeLen;
            try
            {
                conManager.OpenDataReader(sql, out reader);
