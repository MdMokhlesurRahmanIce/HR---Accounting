jQuery(document).ready
		(
			function () {
			    jQuery('#grdDayLeaveApproval').jqGrid
				(
					{
					    url: rootPath + 'GridHelperClasses/GridGenericHandler.ashx?jqGridID=grdDayLeaveApproval&SessionVarName=LeaveTransactionApproval_DayLeaveApproval'
						, editurl: rootPath + 'GridHelperClasses/GridGenericHandler.ashx?jqGridID=grdDayLeaveApproval&editMode=1&SessionVarName=LeaveTransactionApproval_DayLeaveApproval'
						, datatype: 'json'
						, page: 1
						, colNames: ['VID', 'Code', 'Name', 'Designation', 'Leave Type', 'From', 'To', 'Days', 'Reason', 'Forwarded', 'Recomended', 'Approved', 'Rejected']
						, colModel:
							[
								...
								{ 'name': 'ApprovedFrom', 'index': 'ApprovedFrom', 'width': 90, 'editable': true },
								{ 'name': 'ApprovedTo', 'index': 'ApprovedTo', 'width': 90, 'editable': true },
						...
						, rownumbers: true
						, scrollrows: true
						, pager: jQuery('#grdDayLeaveApproval_pager')
...
function addCheckBox_Forwarded() {
    ...
    var ids = jQuery("#grdDayLeaveApproval").jqGrid('getDataIDs');
    for (var i = 0; i < ids.length; i++) {
        ...
        jQuery("#grdDayLeaveApproval").jqGrid('setRowData', ids[i], { IsForwarded: chk });
    ...
    for (var i = 0; i < ids.length; i++) {
        ...
        jQuery("#grdDayLeaveApproval").jqGrid('setRowData', ids[i], { IsRecomended: chk });
        // AllcheckOrUncheck(ColumnName);
    }
    ...
    for (var i = 0; i < ids.length; i++) {
        ...
        jQuery("#grdDayLeaveApproval").jqGrid('setRowData', ids[i], { IsApproved: chk });
        // AllcheckOrUncheck(ColumnName);
    }
    ...
    for (var i = 0; i < ids.length; i++) {
        ...
        jQuery("#grdDayLeaveApproval").jqGrid('setRowData', ids[i], { IsRejected: chk });
        // AllcheckOrUncheck(ColumnName);
    }
}